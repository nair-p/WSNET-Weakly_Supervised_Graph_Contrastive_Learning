...
def train(data, num_epochs, num_splits, config):
    ...
    for fold, (train_mask, test_mask) in enumerate(kf.split(node_indices)):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            train_f1 = f1_score(data.y[train_mask], pred_labels, average='weighted')
...
if __name__=='__main__':
    ...
    full_results = {}
    for p_c, weak_label in data.synth_weak_labels.items():
        print("\n", str(p_c))
        data.weak_labels = data.synth_weak_labels[p_c]
        ...
        mean_res = pretty_print_results(all_results, metric='f1-score', result_level='weighted avg')
        full_results[p_c] = all_results
  
    pkl.dump(full_results, open(args.results_path,'wb'))